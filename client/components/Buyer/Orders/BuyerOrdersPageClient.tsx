...
import {
  fmtDZD, fmtDate, fmtDateTime, parseFarmName,
  ORDER_STATUS_BADGE, ORDER_STATUS_LABEL, ORDER_STATUS_ICON,
} from "@/types/BuyerDashboard";
import { buyerApi, ApiError } from "@/lib/api";

const PAGE_SIZE = 8;
const STATUS_FILTERS = ["all", "pending", "confirmed", "in_transit", "delivered", "cancelled"] as const;
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${ORDER_STATUS_BADGE[status] ?? "bg-slate-100 text-slate-500"}`}>
      <span className="material-symbols-outlined text-[12px] leading-none">{ORDER_STATUS_ICON[status] ?? "help"}</span>
...
function OrderDetailPanel({
  order, onClose, onCancel, isCancelling,
}: {
  order: ApiOrder;
  onClose:     () => void;
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-neutral-dark h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="sticky top-0 bg-white dark:bg-neutral-dark border-b border-neutral-light dark:border-border-dark px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-xs text-slate-500">{fmtDateTime(order.created_at)}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-neutral-50 dark:hover:bg-earth-800 transition-colors" aria-label="Close">
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div className="bg-neutral-50 dark:bg-earth-800 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-0.5">Farm</p>
              <p className="font-bold text-slate-800 dark:text-slate-100">{parseFarmName(order.farm)}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {order.items.map(item => (
                <div key={item.id} className="flex items-center justify-between gap-4 p-3 bg-neutral-50 dark:bg-earth-800 rounded-xl">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm text-slate-800 dark:text-slate-100 truncate capitalize">
                      {item.product.title}
                    </p>
                    <p className="text-[10px] text-slate-400 capitalize">
                      {item.product.category_name ?? "—"} · {item.product.season} · ×{item.quantity} units
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-bold text-sm font-mono">{fmtDZD(item.total_price)}</p>
          ...
          <div className="bg-neutral-50 dark:bg-earth-800 rounded-xl p-4 flex justify-between items-center">
            ...
            <p className="text-2xl font-extrabold font-mono text-slate-900 dark:text-slate-100">
              {fmtDZD(total)} <span className="text-sm font-normal text-slate-400">DZD</span>
            </p>
          </div>
...
export default function BuyerOrdersPage() {
  ...
  const fetchOrders = useCallback(() => {
    ...
    setIsLoading(true);
    setError(null);
    buyerApi.orders(page, PAGE_SIZE, statusFilter === "all" ? undefined : statusFilter)
      .then(res => {
  ...
  const handleCancel = useCallback(async (id: number) => {
    ...
    try {
      ...
      setToast(err instanceof ApiError ? err.message : "Failed to cancel order.");
      setTimeout(() => setToast(null), 3000);
    } finally {
    ...
    <BuyerLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">Orders & Invoices</h1>
        ...
        {error && (
          <div role="alert" className="flex items-center gap-3 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
            ...
            <button onClick={fetchOrders} className="underline font-bold text-xs">Retry</button>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-neutral-dark rounded-2xl border border-neutral-light dark:border-border-dark shadow-sm overflow-hidden">
          {isLoading ? (
            <div className="p-4 space-y-3">
              {Array.from({length: PAGE_SIZE}).map((_,i) => <Skeleton key={i} className="h-20" />)}
            </div>
          ) : orders.length === 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                      order.status === "delivered" ? "bg-primary/10" : "bg-neutral-100 dark:bg-earth-800"
                    }`}>
                      <span className={`material-symbols-outlined text-[18px] ${
                        order.status === "delivered" ? "text-primary" : "text-slate-400"
                      }`}>{ORDER_STATUS_ICON[order.status] ?? "receipt_long"}</span>
                    </div>

                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-sm text-slate-800 dark:text-slate-100">Order #{order.id}</p>
                        <StatusBadge status={order.status} />
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5 truncate">
                        {farmName} · {firstTitle}{extraItems > 0 ? ` +${extraItems} more` : ""} · {fmtDate(order.created_at)}
                      </p>
                    </div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      {order.status === "delivered" && (
                        <p className="text-[10px] text-primary font-bold">Invoice ready</p>
                      )}
                    </div>
          ...
          {totalPages > 1 && (
            <div className="px-5 py-4 border-t border-neutral-light dark:border-border-dark flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button disabled={page === totalPages || isLoading} onClick={() => setPage(p => p + 1)}
