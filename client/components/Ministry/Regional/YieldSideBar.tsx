import type { RegionalAlert } from "@/types/Regional";
import { heatmapGrid, heatmapTitles, regionalAlerts } from "@/types/Regional";

const alertStyles: Record<RegionalAlert["severity"], { wrapper: string; icon: string; title: string; msg: string }> = {
  danger: {
...
export default function YieldSidebar() {
  return (
    <aside className="w-80 bg-surface-light border-r border-neutral-light flex flex-col z-10 overflow-y-auto shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      <div className="p-6">
        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">
          Yield Analytics
        </h2>

        {/* National Yield KPI card */}
        <div className="bg-linear-to-br from-slate-900 to-slate-800 text-white p-5 rounded-xl shadow-lg mb-6 relative overflow-hidden">
          ...
          <p className="text-slate-400 text-xs font-medium mb-1">National Yield Forecast</p>
          <div className="flex items-baseline gap-2 mb-2">
            <span className="text-3xl font-bold">8.4M</span>
            <span className="text-sm text-primary font-medium">+12.5%</span>
          </div>
          <p className="text-xs text-slate-400">Tons expected vs. Q3 2022</p>
          ...
          <div className="space-y-3">
            {regionalAlerts.map((alert) => {
              ...
              return (
                ...
                >
                  <span className={`material-icons text-sm mt-0.5 ${s.icon}`}>{alert.icon}</span>
                  <div>
